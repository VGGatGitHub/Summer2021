...
helper_concept_id_to_index_names_map = {
    ...
    'cItem': ['id_of_Plants_2'],
    'plants': ['id_of_Plants_2'],
    'Plants': ['id_of_Plants']}
helper_property_id_to_column_names_map = {
    ...
    'plants.Id': 'Id',
    'plants.Profit': 'Profit',
    'plants.Product': 'Product'}


# Data model definition for each table
# Data collection: list_of_Plants ['id']
# Data collection: list_of_CustomerDemand ['Product', 'Demand']
# Data collection: list_of_Plants_2 ['Capacity', 'Profit', 'Product', 'Plants', 'Id', 'Cost']

# Create a pandas Dataframe for each data table
# --- Handling table for implicit concept
list_of_Plants = pd.DataFrame(inputs[u'plants']['Plants'].unique(), columns=['id']).dropna()
...
list_of_Plants_2 = inputs[u'plants']
list_of_Plants_2 = list_of_Plants_2[[u'Capacity', u'Profit', u'Product', u'Plants', u'Id', u'Cost']].copy()
list_of_Plants_2.rename(columns={u'Capacity': 'Capacity', u'Profit': 'Profit', u'Product': 'Product', u'Plants': 'Plants', u'Id': 'Id', u'Cost': 'Cost'}, inplace=True)
...
# --- Handling data checking for implicit concept: Plants
...
data_check_result &= helper_check_data_type(list_of_Plants_2, 'Capacity', 'plants', 'NUMBER')
data_check_result &= helper_check_data_type(list_of_Plants_2, 'Profit', 'plants', 'NUMBER')
data_check_result &= helper_check_foreignKey_values(list_of_Plants_2, 'Product', 'plants', list_of_CustomerDemand, 'Product', 'customerDemand')
data_check_result &= helper_check_foreignKey_values(list_of_Plants_2, 'Plants', 'plants', list_of_Plants, 'id', 'Plants')
data_check_result &= helper_check_data_type(list_of_Plants_2, 'Id', 'plants', 'NUMBER')
data_check_result &= helper_check_data_type(list_of_Plants_2, 'Cost', 'plants', 'NUMBER')
data_check_result &= helper_check_unique_primaryKey_values(list_of_Plants_2, ['Id'], 'plants')
if not data_check_result:
    # Stop execution here
    raise Exception('Data checking detected errors')

# Set index when a primary key is defined
list_of_Plants.set_index('id', inplace=True)
...
list_of_Plants_2.set_index('Id', inplace=True)
list_of_Plants_2.sort_index(inplace=True)
list_of_Plants_2.index.name = 'id_of_Plants_2'
...
def build_model():
    ...
    list_of_Plants_2['allocationVar'] = mdl.integer_var_list(len(list_of_Plants_2))
    list_of_Plants_2['selectionVar'] = mdl.binary_var_list(len(list_of_Plants_2))
    ...
    # 	cMaximizeGoalSelect cMaximizeGoalSelect 1.2{
    # 	numericExpr = cAllocation[plants] / plants / Profit,
    # 	scaleFactorExpr = 1,
    # 	(static) goalFilter = null} with weight 7.0
    agg_Plants_2_allocationVar_SG1 = mdl.sum(list_of_Plants_2.allocationVar)
    list_of_Plants_2['conditioned_Cost'] = list_of_Plants_2.allocationVar * list_of_Plants_2.Cost
    agg_Plants_2_conditioned_Cost_SG2 = mdl.sum(list_of_Plants_2.conditioned_Cost)
    list_of_Plants_2['conditioned_Profit'] = list_of_Plants_2.allocationVar * list_of_Plants_2.Profit
    agg_Plants_2_conditioned_Profit_SG3 = mdl.sum(list_of_Plants_2.conditioned_Profit)
    
    kpis_expression_list = [
        (-1, 512.0, agg_Plants_2_allocationVar_SG1, 1, 0, u'total plants allocations'),
        (1, 16.0, agg_Plants_2_conditioned_Cost_SG2, 1, 0, u'total Cost of plants over all allocations'),
        (-1, 64.0, agg_Plants_2_conditioned_Profit_SG3, 1, 0, u'total Profit of plants over all allocations')]
    ...
    join_Plants_2 = list_of_Plants_2.reset_index().merge(list_of_Plants_2.reset_index(), left_on=['id_of_Plants_2'], right_on=['id_of_Plants_2'], suffixes=('', '_right')).set_index(['id_of_Plants_2'])
    groupbyLevels = ['id_of_Plants_2']
    groupby_Plants_2 = join_Plants_2.allocationVar.groupby(level=groupbyLevels[0]).sum().to_frame(name='allocationVar')
    list_of_Plants_2['conditioned_Capacity'] = list_of_Plants_2.selectionVar * list_of_Plants_2.Capacity
    join_Plants_2_2 = groupby_Plants_2.join(list_of_Plants_2.conditioned_Capacity, how='inner')
    for row in join_Plants_2_2.itertuples(index=True):
    ...
    groupbyLevels = ['id_of_Plants_2']
    groupby_Plants_2 = join_Plants_2.allocationVar.groupby(level=groupbyLevels[0]).sum().to_frame(name='allocationVar')
    list_of_Plants_2_minValueAllocationForAssignment = pd.Series([0] * len(list_of_Plants_2)).to_frame('minValueAllocationForAssignment').set_index(list_of_Plants_2.index)
    join_Plants_2_2 = list_of_Plants_2.join(list_of_Plants_2_minValueAllocationForAssignment.minValueAllocationForAssignment, how='inner')
    join_Plants_2_2['conditioned_minValueAllocationForAssignment'] = join_Plants_2_2.selectionVar * join_Plants_2_2.minValueAllocationForAssignment
    join_Plants_2_3 = groupby_Plants_2.join(join_Plants_2_2.conditioned_minValueAllocationForAssignment, how='inner')
    for row in join_Plants_2_3.itertuples(index=True):
    ...
    for row in list_of_Plants_2[list_of_Plants_2.Capacity.notnull()].itertuples(index=True):
    ...
    join_CustomerDemand = list_of_CustomerDemand[[]].reset_index().merge(list_of_Plants_2.reset_index(), left_on=['id_of_CustomerDemand'], right_on=['Product']).set_index(['id_of_CustomerDemand', 'id_of_Plants_2'])
...
def export_solution(msol):
    ...
    mdl = msol.model
    list_of_Plants_2_solution = pd.DataFrame(index=list_of_Plants_2.index)
    list_of_Plants_2_solution['allocationVar'] = msol.get_values(list_of_Plants_2.allocationVar.values)
    list_of_Plants_2_solution = list_of_Plants_2_solution.round({'allocationVar': 2})
    list_of_Plants_2_solution['selectionVar'] = msol.get_values(list_of_Plants_2.selectionVar.values)
    plantsAllocation = pd.DataFrame(index=list_of_Plants_2.index)
    
    # Adding extra columns based on Solution Schema
    plantsAllocation['plants allocation decision'] = list_of_Plants_2_solution['allocationVar']
    plantsAllocation['plants selection decision'] = list_of_Plants_2_solution['selectionVar']
    plantsAllocation['plants Capacity'] = list_of_Plants_2['Capacity']
    plantsAllocation['plants Profit'] = list_of_Plants_2['Profit']
    plantsAllocation['plants Product'] = list_of_Plants_2['Product']
    plantsAllocation['plants Plants'] = list_of_Plants_2['Plants']
    plantsAllocation['plants Cost'] = list_of_Plants_2['Cost']
    list_of_Plants_2_minValueAllocationForAssignment = pd.Series([0] * len(list_of_Plants_2)).to_frame('minValueAllocationForAssignment').set_index(list_of_Plants_2.index)
    join_Plants_2 = list_of_Plants_2.join(list_of_Plants_2_minValueAllocationForAssignment.minValueAllocationForAssignment, how='inner')
    plantsAllocation['plants minValueAllocationForAssignment'] = join_Plants_2['minValueAllocationForAssignment']
